...
if not os.path.isfile(output_csv_file):
    with open(output_csv_file, "a+") as csv_file:
        ...
        headers = jtl_headers + gc_headers + perf_headers+ cpu_sar_headers+ memory_sar_headers+ swap_sar_headers+io_sar_headers+inode_sar_headers+context_switch_sar_headers + run_queue_sar_headers+ network_sar_headers
        writer.writerow(headers)

with open(output_csv_file, "a+") as csv_file:
    writer = csv.writer(csv_file, delimiter=',')
    values =  jtl_values + gc_values + perf_values + cpu_sar_values + memory_sar_values+ swap_sar_values + io_sar_values + inode_sar_values + context_switch_sar_values + run_queue_sar_values + network_sar_values
