...
from gc_stats import *
from perf_stats import *
from sar_stats import *
...
run_time_seconds= sys.argv[12]

architecture= sys.argv[13]
use_case= sys.argv[14]
heap= sys.argv[15]
u= sys.argv[16]
gc= sys.argv[17]
size= sys.argv[18]
...
basic_values = [architecture, use_case, heap, u, gc, size]
...
if not os.path.isfile(output_csv_file):
    with open(output_csv_file, "a+") as csv_file:
        ...
        headers = basic_headers+ jtl_headers + gc_headers + cpu_sar_headers+ memory_sar_headers+ swap_sar_headers+io_sar_headers+inode_sar_headers+context_switch_sar_headers + run_queue_sar_headers+ network_sar_headers
        writer.writerow(headers)

with open(output_csv_file, "a+") as csv_file:
    writer = csv.writer(csv_file, delimiter=',')
    values =  basic_values + jtl_values + gc_values + cpu_sar_values + memory_sar_values+ swap_sar_values + io_sar_values + inode_sar_values + context_switch_sar_values + run_queue_sar_values + network_sar_values
